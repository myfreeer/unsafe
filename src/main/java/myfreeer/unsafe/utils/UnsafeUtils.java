...
import myfreeer.unsafe.utils.factory.MethodHandleUnsafeFactory;
import myfreeer.unsafe.utils.invoke.LookupFactory;

import java.lang.invoke.MethodHandles;

public class UnsafeUtils {
    private static volatile MethodHandleUnsafeFactory factory = null;
    ...
    public static BaseUnsafeFactory getUnsafeFactory() {
        ...
        if (factory == null) {
            synchronized (UnsafeUtils.class) {
                ...
                if (factory == null) {
                    try {
                        factory = new MethodHandleUnsafeFactory();
    ...
    public static Accessor getAccessor() {
        ...
        if (accessor == null) {
            synchronized (Accessor.class) {
                ...
                if (accessor == null) {
                    try {
                        accessor = new UnsafeAccessor(factory);
                    } catch (UnsafeException e) {
                        accessorFailed = true;
                    }
                }
